...
private:
    ...
    int							m_iMotorValue;
    bool						m_Align;
    std::vector<OpenNICapture*>                captures;
    bool bRecording;
};
...
MultipleKinectApp::~MultipleKinectApp()
{
    ...
    for (uint i = 0; i < m_2RealKinect->getNumberOfDevices();i++){
...
void MultipleKinectApp::setup()
{
	...
	try
	{
		...
		for ( int devIdx=0; devIdx < m_iNumberOfDevices ; ++devIdx )
		{
            ...
            captures.push_back(new OpenNICapture());
		}
		resizeImages();
	}
...
{
    if(bRecording){
        for (uint i = 1; i < m_2RealKinect->getNumberOfDevices(); i++){
            captures[i]->update();
        }
	}
}
...
void MultipleKinectApp::mouseDown(cinder::app::MouseEvent event){
...
void MultipleKinectApp::keyDown( KeyEvent event )
{
    ...
    if(event.getChar()=='r'){
		...
		if(bRecording){
            for (uint i = 1; i < m_2RealKinect->getNumberOfDevices(); i++){
                captures[i]->setup(m_2RealKinect,i);
                captures[i]->startCapture();
            }
		}else{
            for (uint i = 1; m_2RealKinect->getNumberOfDevices(); i++){
                captures[i]->stopCapture();
            }
		}
	}
