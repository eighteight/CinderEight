...
public:
	...
	void addRandomSpring() {
		Physics::Particle3D *a = physics.getParticle((int)Rand::randFloat(0, physics.numberOfParticles()));
		Physics::Particle3D *b = physics.getParticle((int)Rand::randFloat(0, physics.numberOfParticles()));
	...
	void killRandomParticle() {
		Physics::Particle3D *p = physics.getParticle(floor(Rand::randFloat(0, physics.numberOfParticles())));
		if(p && p != &mouseNode) p->kill();
	}
	
	void killRandomSpring() {
		Physics::Spring3D *s = physics.getSpring( floor(Rand::randFloat(0, physics.numberOfSprings())));
		if(s) s->kill();
	}
	
	void killRandomConstraint() {
		Physics::Constraint3D *c = physics.getConstraint(floor(Rand::randFloat(0, physics.numberOfConstraints())));
		if(c) c->kill();
	}
	...
	void draw() {
		...
		if(doRender) {
			...
			for(int i=0; i<physics.numberOfSprings(); i++) {
				...
				float size  = mapRange(spring->getStrength(), SPRING_MIN_STRENGTH, SPRING_MAX_STRENGTH, SPRING_MIN_WIDTH, SPRING_MAX_WIDTH);
			...
			for(int i=0; i<physics.numberOfParticles(); i++) {
				...
				float alpha = mapRange(p->getPosition().y, -height * 1.5f, height, 0.0f, 1.0f);
				if(alpha>0) {
					glPushMatrix();
