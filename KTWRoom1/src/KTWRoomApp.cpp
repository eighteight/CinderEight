...
class KTWRoomApp : public AppBasic {
  public:
...
private: 
    std::shared_ptr<BallSystem> ballSystemRef;
...
void KTWRoomApp::prepareSettings( Settings *settings )
{
	settings->setWindowSize( APP_WIDTH, APP_HEIGHT );
	//settings->setBorderless( true );
}

void KTWRoomApp::setup()
{	
    ballSystemRef = std::shared_ptr<BallSystem>(new BallSystem());
    ballSystemRef->setup(-2000);
	// CAMERA	
...
void KTWRoomApp::mouseDown( MouseEvent event )
{
...
void KTWRoomApp::mouseUp( MouseEvent event )
{
	mMousePressed = false;
	mMouseOffset = Vec2f::zero();
}

void KTWRoomApp::mouseMove( MouseEvent event )
{
	mMousePos = event.getPos();
}

void KTWRoomApp::mouseDrag( MouseEvent event )
{
	mouseMove( event );
	mMouseOffset = ( mMousePos - mMouseDownPos ) * 0.4f;
}

void KTWRoomApp::mouseWheel( MouseEvent event )
{
	float dWheel	= event.getWheelIncrement();
	mRoom.adjustTimeMulti( dWheel );
}

void KTWRoomApp::keyDown( KeyEvent event )
{
    ...
    ballSystemRef->keyDown(event);
}



void KTWRoomApp::update()
{	
	// ROOM
    ...
    ballSystemRef->update();
}

void KTWRoomApp::drawIntoRoomFbo()
{
...
//	}
    ...
    gl::translate( Vec3f( 0.0f, 100.0f, 0.0f ) );

    ballSystemRef->draw();
}

void KTWRoomApp::drawInfoPanel()
{
	gl::pushMatrices();
...
CINDER_APP_BASIC( KTWRoomApp, RendererGl )
