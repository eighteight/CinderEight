...
inline cv::Vec2f fromOcv( const cv::Point2f &point )
{
	return  cv::Vec2f( point.x, point.y );
}

inline cv::Point2f toOcv( const cv::Vec2f &point )
{
	return cv::Point2f( point[0], point[0]);
}

inline cv::Vec2i fromOcv( const cv::Point &point )
{
	return  cv::Vec2i( point.x, point.y );
}

inline cv::Point toOcv( const cv::Vec2i &point )
{
