...
static float one_over_640 = 1.0f/640.0f;
static float one_over_480 = 1.0f/480.0f;
...
**/
    ...
    bool OpenNIUser::isTracking(){
        return mUserState == USER_TRACKING;
    }

    using namespace cinder;
    void OpenNIUser::updatePosition(int joint,  boost::unordered_map<int, std::vector<cinder::Vec3f> > &positions){
        OpenNIBone* b = mBoneList[joint];
        positions[mId][joint] = Vec3f(b->position[0],b->position[1], b->position[2]);
    }

    void OpenNIUser::updatePositions(boost::unordered_map<int, std::vector<cinder::Vec3f> > &positions){
        if (!isTracking()) return;
        if (!positions.count(getId())){
            positions.insert (std::make_pair<int,std::vector <cinder::Vec3f> >(getId(),std::vector<cinder::Vec3f> (mBoneList.size(), cinder::Vec3f())) );
        }
        for(int i=0; i<mBoneList.size(); i++)
        {
            updatePosition(i, positions);
        }
    }
...
//				return;
			...
			for( std::vector<OpenNIBone*>::iterator it = mBoneList.begin(); it != mBoneList.end(); ++it, index++ )
			{
                ...
                point.X *= one_over_640;			// div by 640
				point.Y *= one_over_480;			// div by
				float sx = pointSize;
				float sy = pointSize;
            ...
            renderBone( SKEL_LEFT_SHOULDER, SKEL_NECK, width, height, depth, true, renderDepth );
            renderBone( SKEL_RIGHT_SHOULDER, SKEL_NECK, width, height, depth, true, renderDepth );
	...
	void OpenNIUser::renderJointsRealWorld( float pointSize, float zScale )
	{
		...
		//if( !mBoneList.empty() && _device->getUserGenerator()->GetSkeletonCap().IsTracking(mId) )
		{
			...
			renderBone( SKEL_LEFT_SHOULDER, SKEL_TORSO, 0, 0, 1, false, false  );
			renderBone( SKEL_RIGHT_SHOULDER, SKEL_TORSO, 0, 0, 1, false, false  );
			...
			renderBone( SKEL_TORSO, SKEL_RIGHT_HIP, 0, 0, 1, false, false );
			renderBone( SKEL_TORSO, SKEL_LEFT_HIP, 0, 0, 1, false, false );
            renderBone( SKEL_LEFT_HIP, SKEL_RIGHT_HIP, 0, 0, 1, false, false );
	...
	void OpenNIUser::renderBone( int joint1, int joint2, float width/*=640*/, float height/*=480*/, float depth/*=1*/, bool doProjective /*= true*/, bool renderDepthInProjective/*=false */ )
	{
		...
		if( doProjective )
		{
			...
			point1.X *= one_over_640;			// div by 640
			point1.Y *= one_over_480;			// div by 480
			point2.X *= one_over_640;			// div by 640
			point2.Y *= one_over_480;			// div by 480
			point1.Z *= depth;
			point2.Z *= depth;
