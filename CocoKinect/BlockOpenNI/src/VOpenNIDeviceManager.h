...
//        OpenNIDeviceRef         createDevice( const std::string& xmlFile, bool allocUserIfNoNode=false );
		...
		bool                    createDevices( uint32_t deviceCount, int nodeTypeFlags, int colorResolution=RES_640x480, int depthResolution=RES_640x480 );
        bool createDevice(const XnChar* strFileName, int nodeTypeFlags, int deviceNum);
        ...
        void updatePositions(boost::unordered_map<int, std::vector<cinder::Vec3f> > &);
	...
	protected:
		...
		//
	public:
                void            setFrame(size_t, uint);
                size_t          getTotalNumFrames(uint);
