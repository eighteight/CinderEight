...
int AudioDevice::createSound(SoundItemRef pSoundItem){
	...
	int result = mSystem->createSound(("/Users/eight/repos/Cinder/CinderEight/AudioEngine/assets/sounds/"+mSounds[mSounds.size()-1]->filename).c_str(), FMOD_3D, 0, &mSounds[mSounds.size()-1]->sound);
	ERRCHECK((FMOD_RESULT)result);
