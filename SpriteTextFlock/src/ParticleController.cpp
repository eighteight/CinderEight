...
void ParticleController::update( bool flatten, std::string* currentWrd )
{
	for( list<Particle>::iterator p = mParticles.begin(); p != mParticles.end(); ++p ){
		p->update( flatten, currentWrd  );
	}
}
...
void ParticleController::draw(cinder::gl::TextureFontRef* textureFont)
{
	gl::color( ColorA( 1.0f, 1.0f, 1.0f, 1.0f ) );
	for( list<Particle>::iterator p = mParticles.begin(); p != mParticles.end(); ++p ){
		p->draw(textureFont);
...
void ParticleController::addParticles( int amt )
{
	for( int i=0; i<amt; i++ )
	{
		...
		Vec3f pos = randVec * Rand::randFloat( 200.0f, 600.0f );
		Vec3f vel = -randVec;
