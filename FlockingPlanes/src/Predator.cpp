...
vec2 safeNormalized(vec2 what)
{
    float length = what.length();
    if( length > 0 ) {
        return what / length;
    }
    else
...
void Predator::pullToCenter( const vec3 &center )
{
	...
	if( distToCenter > maxDistance ){
		...
		mVel -= safeNormalized(dirToCenter) * ( ( distToCenter - maxDistance ) * pullStrength );
	}
}	
...
{	
	...
	mVel += mAcc;
	mVelNormal = safeNormalized(mVel);
...
void Predator::limitSpeed()
{
	...
	float vLengthSqrd = mVel.length();
    vLengthSqrd *= vLengthSqrd;
