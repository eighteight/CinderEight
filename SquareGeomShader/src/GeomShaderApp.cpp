...
private:
	...
	// Mesh parameters
	int32_t mMeshLength, mMeshWidth;
...
void GeomShaderApp::keyDown(ci::app::KeyEvent event){
    if (event.getChar() == 'w'){
        isWriting = !isWriting;
    }
}
...
void GeomShaderApp::draw()
{
    ...
    mShader.uniform("aspect", ((float)getWindowWidth())/(float)getWindowHeight());
...
void GeomShaderApp::setup()
{
	...
	// Load shader
	mShader = gl::GlslProg(loadAsset("draw.vert"), loadAsset("draw.frag"), loadAsset("draw.geom"), GL_POINTS, GL_TRIANGLE_STRIP, maxGeomOutputVertices);
	...
	mMeshScale = 10.0f;
	mMeshWaveAmplitude = (float)M_PI*0.25;
	mMeshWaveSpeed = 0.3;//0.667f;
	mMeshWaveWidth = 0.1005;//0.067f;

	
	// Set default triangle dimensions
	mTriangleRotationSpeed = 0.0; //0.106667f;
	mTriangleSize = 40.0f;
	...
	mParams.addParam("Width", & mMeshWidth, "min=1 max=2048 step=1");
	//mParams.addButton("Reset", std::bind(& GeomShaderApp::initMesh, this), "key=r");
	mParams.addSeparator("");
	mParams.addText("TRIANGLE");
	mParams.addParam("Rotation speed", & mTriangleRotationSpeed, "min=0.0 max=300.0 step=0.001 keyDecr=t keyIncr=T");
	...
	if( qtime::MovieWriter::getUserCompressionSettings( &format, loadImage( loadAsset( "texture.jpg" ) ) ) ) {
